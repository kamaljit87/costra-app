...
export default function CancelEmailChangePage() {
  ...
  useEffect(() => {
    ...
    const cancel = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/profile/cancel-email-change`, {
          method: 'POST',
        ...
        if (res.ok) {
          setStatus('success')
          setMessage(data.message || 'Email change cancelled.')
        } else {
          setStatus('error')
        ...
        <div className="w-full max-w-md rounded-2xl bg-white border border-surface-200 shadow-xl p-8 text-center">
          {status === 'loading' && (
            <>
              ...
              <h1 className="text-xl font-semibold text-gray-900 mb-2">Cancelling email change...</h1>
            </>
          )}
          {status === 'success' && (
            <>
              ...
              <h1 className="text-xl font-semibold text-gray-900 mb-2">Email Change Cancelled</h1>
